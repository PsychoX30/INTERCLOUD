...
const LedgerPane = ({ rows, onChange, kind, extras, rowAction, useItems = false }) => {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setErr("");
    const payload = useItems ? { ...form, items } : form;
    if (useItems && itemsTotal === 0) { setErr("Jumlah total tidak boleh 0."); return; }
    try {
      ...
      {adding && (
        <Card className="p-4 mb-3">
          ...
          <form onSubmit={submit} className="grid grid-cols-2 md:grid-cols-4 gap-2">
            ...
            {!useItems && <label><div className={labelClass}>Amount (IDR)</div><input type="number" required value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} className={inputClass} data-testid={`${kind}-amount`} /></label>}
            {extras.map((k) => (
            ...
            {useItems && (
              <div className="col-span-full border-t border-slate-100 pt-3 mt-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addItem} className="text-xs font-bold text-[#0a2350] hover:text-[#f5b120]" data-testid={`${kind}-add-item`}><Plus className="h-3.5 w-3.5 inline" /> Tambah baris</button>
                </div>
                {items.map((it, i) => (
                  <div key={i} className="grid grid-cols-[1fr_140px_32px] gap-2 mb-2 items-end">
                    <label><div className={labelClass}>Keterangan</div><input value={it.description} onChange={(e) => setItem(i, "description", e.target.value)} className={inputClass} placeholder="e.g. Gaji pokok / BPJS / Pajak" data-testid={`${kind}-item-${i}-desc`} /></label>
                    <label><div className={labelClass}>Nominal</div><input type="number" value={it.amount} onChange={(e) => setItem(i, "amount", Number(e.target.value))} className={inputClass} data-testid={`${kind}-item-${i}-amount`} /></label>
                ...
                <div className="text-right text-sm mt-1">Total: <b className="text-[#0a2350]" data-testid={`${kind}-items-total`}>{idr(itemsTotal)}</b></div>
              </div>
            )}

...
export const BlackholeLog = () => {
  ...
  useEffect(() => {
    api.get("/admin/noc/blackhole-log", { params: { limit: 200 } })
      .then((r) => setLog(r.data))
      .catch(() => setLog([]));
  }, []);

  const rows = (log || []).filter((l) => !q || l.prefix.includes(q) || (l.by || "").toLowerCase().includes(q.toLowerCase()) || (l.device || "").toLowerCase().includes(q.toLowerCase()));
    ...
    <Card className="overflow-hidden mb-6" data-testid="blackhole-log">
      ...
      {log === null ? (
        ...
        <div className="px-5 py-6 text-sm text-slate-500" data-testid="blackhole-log-empty">
          {log.length === 0 ? "Belum ada aktivitas blackhole tercatat." : "Tidak ada entri yang cocok."}
        </div>
      ) : (

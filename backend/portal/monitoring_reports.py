...
def _graph_export_pdf_bytes(graph: dict, rows: list[dict]) -> bytes:
    ...
    # A merged traffic pair carries "in"/"out" per row instead of a single
    # "value"; render it as two throughput columns so one PDF covers both
    # directions of the interface.
    is_pair = bool(rows) and "in" in rows[0] and "out" in rows[0]

    body_rows = []
    if is_pair:
        headers = "<tr><th>Timestamp</th><th>IN</th><th>OUT</th></tr>"
        col_count = 3
        for row in rows:
            at = row.get("at")
            at_str = at.strftime("%Y-%m-%d %H:%M:%S") if hasattr(at, "strftime") else escape(str(at))
            in_str = format_sample_value(row.get("in"), raw_unit, raw_type)
            out_str = format_sample_value(row.get("out"), raw_unit, raw_type)
            body_rows.append(
                f"<tr><td>{escape(at_str)}</td><td>{escape(in_str)}</td>"
                f"<td>{escape(out_str)}</td></tr>"
            )
    else:
        headers = "<tr><th>Timestamp</th><th>Value</th><th>Min</th><th>Max</th></tr>"
        col_count = 4
        for row in rows:
            at = row.get("at")
            at_str = at.strftime("%Y-%m-%d %H:%M:%S") if hasattr(at, "strftime") else escape(str(at))
    ...
    empty_row = f'<tr><td colspan="{col_count}">No data in range.</td></tr>'
...
# ---------------------------------------------------------------------------
def graph_export_response(graph: dict, rows: list[dict]):
    """Return a StreamingResponse for the given graph data as a PDF report."""
